...
class OpenImagesDataset(data.Dataset):
    def __init__(self, img_folder, annotate_file, labels, transforms=None):
        self.nb_boxes = 0
        self.images_id = []
        self.images_data = self._read_data_info(img_folder, annotate_file, labels)
        self.transforms = transforms
       
        
    def _read_data_info(self, img_folder, annotate_file, labels) -> dict:
        images_list = os.listdir(img_folder)
        df = pd.read_csv(annotate_file)
        df_filtered = df[df["LabelName"].isin(labels)]
        df_occluded = df_filtered[df_filtered["IsOccluded"] == 0]
        df_cleaned = df_occluded[df_occluded["IsTruncated"] == 0]
        
        images_data = {}
        for img in images_list:
            imageID = img.replace(".jpg", "")
            df_name = df_cleaned[df_cleaned["ImageID"] == imageID]
            if df_name.empty:
                continue           
            self.images_id.append(imageID)
            bbox_info = []
            image_info = [
                os.path.join(img_folder, img),
                imageID
            ]
            for _, dfRow in df_name.iterrows():
                row_info = {}
                row_info["label_id"] = labels.index(dfRow["LabelName"]) + 1
                row_info["bBox"] = (
						dfRow["XMin"], dfRow["YMin"], 
						dfRow["XMax"], dfRow["YMax"]
					)
                bbox_info.append(row_info)
                
                self.nb_boxes+=1
            
            images_data[imageID] = {"image_info": image_info, "bbox_info": bbox_info}
    ...
    def __getitem__(self, idx):
        imageID = self.images_id[idx]
        image_data = self.images_data[imageID]
        image_path = image_data["image_info"][0]
                
        image = Image.open(image_path).convert("RGB")
        w, h = image.size
        
        bBoxes = image_data["bbox_info"]
        
        bbox_sizes = []
        bbox_labels = []
        for bBox in bBoxes:
            bbox_sizes.append(bBox["bBox"])
            bbox_labels.append(bBox["label_id"])

        bbox_sizes = torch.tensor(bbox_sizes)
        bbox_labels = torch.tensor(bbox_labels)

        if self.transforms:
            image, (h, w), bbox_sizes, bbox_labels = \
                self.transforms(image, (h, w), bbox_sizes, bbox_labels)

        return image, imageID, (h, w), bbox_sizes, bbox_labels
...
class COCODetection(data.Dataset):
    def __init__(self, img_folder, annotate_file, transform=None):
        ...
        for img in self.data["images"]:
            ...
            if not img_name in os.listdir(self.img_folder):
                continue
            img_id = img["id"]
